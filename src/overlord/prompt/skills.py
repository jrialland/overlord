...
def load_markdown_with_frontmatter(md_path: Path) -> tuple[dict | None, str]:  # type: ignore[type-arg] # FIX ME
    """
    Load a Markdown file with optional YAML frontmatter.

    Returns a tuple of (frontmatter, markdown_body). If the file has no
    frontmatter, ``frontmatter`` is ``None``.
    """
    ...
    if content.startswith("---"):
        ...
        return yaml.safe_load(fm), md.strip()
    else:
        return None, content
...
def skills_descriptions_to_markdown(
    ...
    md += f"_SKILLS are loadable capabilities that can be used to accomplish specific tasks. There {_is} {len(skills)} skill{plural} available._\n\n"
    md += "The following skills extend your capabilities. You can use a skill by invoking the choose_skill tool with the corresponing SKILL_ID.\n"
    md += "Activating a skill increases your chances of successfully completing your task.\n\n"
    md += table_row(["SKILL_ID", "Description", "File location"]) + "\n"
    md += table_delimiter_row(2) + "\n"

    for skill in skills.values():
        skill_path = skill.location.relative_to(workspace_path)
        md += (
            table_row(
                [
